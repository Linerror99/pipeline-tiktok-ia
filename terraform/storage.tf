# Cloud Storage Buckets

# Bucket V1 (Veo 3.0)
resource "google_storage_bucket" "artifacts_v1" {
  name          = var.bucket_name_v1
  location      = var.region
  force_destroy = false
  
  uniform_bucket_level_access = true
  
  versioning {
    enabled = true
  }
  
  lifecycle_rule {
    condition {
      age = 90
    }
    action {
      type = "Delete"
    }
  }
  
  depends_on = [google_project_service.required_apis]
}

# Bucket V2 (Veo 3.1)
resource "google_storage_bucket" "artifacts_v2" {
  name          = var.bucket_name_v2
  location      = var.region
  force_destroy = false
  
  uniform_bucket_level_access = true
  
  versioning {
    enabled = true
  }
  
  lifecycle_rule {
    condition {
      age = 90
    }
    action {
      type = "Delete"
    }
  }
  
  depends_on = [google_project_service.required_apis]
}

# IAM pour les buckets
resource "google_storage_bucket_iam_member" "functions_v1_access" {
  bucket = google_storage_bucket.artifacts_v1.name
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:${google_service_account.cloud_functions_sa.email}"
}

resource "google_storage_bucket_iam_member" "functions_v2_access" {
  bucket = google_storage_bucket.artifacts_v2.name
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:${google_service_account.cloud_functions_sa.email}"
}
